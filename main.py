...
from ui.components import ExpandableSection, DownloadSection
...
def initialize_streamlit():
    ...
    # Updated CSS for better padding and title handling
    st.markdown("""
    <style>
    div.block-container {
        padding-top: 2rem;  # Increased padding to prevent cutoff
    }
    .stApp > header + div [data-testid="stMarkdownContainer"] {
        word-wrap: break-word;  # Enable word wrapping for long titles
...
def create_performance_summary_section(summary_data, title):
    """Create performance summary section with metrics."""
...
def create_data_download_section(df, year, company_group, selected_apgs, available_apgs, show_outliers, method):
    ...
    if show_outliers and method:
        method_key = 'std' if 'STD' in method else ('iqr' if 'IQR' in method else 'mad')
        outlier_columns = [f'filtered_mean_{method_key}', f'outlier_count_{method_key}']
...
def run_standard_analysis(settings, sidebar_manager, apg_filter, file_path):
    ...
    # Pagination setup
    charts_per_page = 50
    total_pages = math.ceil(len(shuffled_df) / charts_per_page)
    page_number = st.selectbox("Select Page", range(1, total_pages + 1), index=0)
    start_idx = (page_number - 1) * charts_per_page
    end_idx = start_idx + charts_per_page
    page_df = shuffled_df.iloc[start_idx:end_idx]
    
    # Create charts for current page
...
def run_cumulative_analysis(settings, sidebar_manager, apg_filter, file_path):
    ...
    # Pagination setup for cumulative mode
    charts_per_page = 50
    total_pages = math.ceil(len(selected_apgs) / charts_per_page)
    page_number = st.selectbox("Select Page", range(1, total_pages + 1), index=1)
    start_idx = (page_number - 1) * charts_per_page
    end_idx = start_idx + charts_per_page
    page_apgs = selected_apgs[start_idx:end_idx]
    
    # Display charts for current page
    chart_builder = ChartBuilder(settings)
    method_key = 'std' if 'STD' in outlier_method else ('iqr' if 'IQR' in outlier_method else 'mad')
    
    col1, col2 = st.columns(2)
    
    for idx, apg_no in enumerate(page_apgs):
